...
let
  ...
  # Directory name drongo looks under for its bundled libsecp256k1.
  drongoArch =
    {
      x86_64-linux = "x64";
      aarch64-linux = "aarch64";
  ...
  sparrow-modules = stdenvNoCC.mkDerivation {
    ...
    buildPhase = ''
      ...
      # As of 2.5.0 upstream ships libsecp256k1 in the bundled runtime's lib
      # directory instead of inside the drongo module. We replace that runtime
      # with the Nixpkgs JDK, so restore the library to the path drongo loads
      # it from as a module resource, otherwise it silently falls back to the
      # much slower pure-Java EC implementation.
      install -Dm755 lib/runtime/lib/libsecp256k1.so \
        modules/com.sparrowwallet.drongo/native/linux/${drongoArch}/libsecp256k1.so
